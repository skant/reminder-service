...
from parsedatetime_consts import pdtLocale_en as pdtConst
...
class Calendar:
    ...
    def _evalString(self, datetimeString, sourceTime=None):
        ...
        @return: calculated C{struct_time} value or current C{struct_time}
                 if not parsed
        """
        s   = self.FilterUnwantedText(datetimeString).strip()
        now = time.localtime()
        ...
        if self.timeStrFlag:
            if s in self.ptc.re_values['now']:
                #sourceTime = now         Commenting to Fix NOW issue 
                pass
            else:
                sources = self.ptc.buildSources(sourceTime)
    ...
    def parse(self, datetimeString, sourceTime=None):
        ...
        s         = self.FilterUnwantedText(datetimeString).strip().lower()
        parseStr  = ''
        ...
        while len(s) > 0:
            ...
            if parseStr != '':
                if self.modifierFlag == True:
                    ...
                    if _debug:
                       print " 5 >> " + str(totalTime)
                    parseStr  = ''

        # String is not parsed at all
        #if totalTime is None or totalTime == sourceTime: // removing "totalTime == sourceTime" to fix NOW issue 
        if totalTime is None:
            #totalTime     = time.localtime() // commenting and adding line below to fix NOW issue 
            totalTime = sourceTime
            if _debug:   
               print " 6 >> " + str(totalTime)
            self.dateFlag = 0
            self.timeFlag = 0
    ...
    def FilterUnwantedText(self, text): 
      
      DAYS ="(?P<days>("+ "|".join(pdtConst.Weekdays) + "|" + "|".join(pdtConst.shortWeekdays) +"))"
      MONTHS ="(?P<months>("+ "|".join(pdtConst.Months) + "|" + "|".join(pdtConst.shortMonths) +"))"
      day_regex_begin = r'([^a-z])'
      day_regex_end   = r'([^a-z])' 
      days_regex = day_regex_begin + DAYS + day_regex_end
      p_day = re.compile(days_regex)
      month_regex_begin = r'((\s.*)|([0-9](st|nd|rd|th))|([^a-z]))'
      month_regex_end   = r'([^a-z])' 
      month_regex = month_regex_begin + DAYS + month_regex_end
      p_month = re.compile(month_regex)
      
      Xs= "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX"
      changed = False
      
      text = "   " + text + "   " # 3 spaces
      text = text.lower()
      for s in re.finditer(MONTHS, text):
         a = s.start()
         b = s.end()
         if p_month.search(text[s.start()-3:s.end()+3]) == None:
            text = text[:s.start()] + Xs[0:s.end()-s.start()] + text[s.end():]
            changed = True
               
      for s in re.finditer(DAYS, text):
         a = s.start()
         b = s.end()
         if p_day.search(text[s.start()-3:s.end()+3]) == None:
            text = text[:s.start()] + Xs[0:s.end()-s.start()] + text[s.end():]
            changed = True
      
      if changed:
         return self.FilterUnwantedText(text)
      else :
         return str.strip(text) #removing the space added in the beginning
